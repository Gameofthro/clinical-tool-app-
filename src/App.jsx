...
import { App } from '@capacitor/app'; // CAPACITOR INTEGRATION
import { 
...
export default function ClinicalTool() {
  ...
  useEffect(() => {
    const backListener = App.addListener('backButton', () => {
      // Priority 1: Close full-screen disease modal
      if (selectedDisease) {
        setSelectedDisease(null);
      } 
      // Priority 2: Close navigation drawer
      else if (isDrawerOpen) {
        setIsDrawerOpen(false);
      } 
      // Priority 3: Close informational modals
      else if (showAboutModal) {
        setShowAboutModal(false);
      } else if (showReferencesModal) {
        setShowReferencesModal(false);
      } 
      // Priority 4: Close non-mandatory legal terms
      else if (showTerms && !mandatoryTerms) {
        setShowTerms(false);
      } 
      // Priority 5: Navigate back to home from tools/search
      else if (activeTab !== "home") {
        setActiveTab("home");
        setQuery("");
      } 
      // Priority 6: If on home screen with nothing open, exit app
      else {
        App.exitApp();
      }
    });
        ...
        <div className="animate-in fade-in slide-in-from-bottom-2 duration-500 flex-1 w-full h-full">
          ...
          {activeTab === "tools" && (
             <div className="space-y-4">
               ...
               {/* Tool Navigation Buttons - ICON ONLY STRIP */}
               <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar border-b border-slate-200 dark:border-slate-700">
                 {[
                    {id: 'pediatric', label: 'Pediatric Dose', icon: Baby}, 
                    {id: 'fluid', label: 'IV Fluids', icon: Droplet}, 
