...
/** * MASTER EXPORTS 
 * Ensure these names match the imports in your Tool files exactly.
 */
...
// RENAME CHECK: This is InputGroup, NOT Input.
  ...
  <div className="flex gap-3 w-full animate-in fade-in duration-500">
    <div className="relative flex items-center flex-1">
      ...
      <input 
        ...
        className="w-full py-5 pl-12 pr-4 bg-slate-100 dark:bg-slate-800/80 border-2 border-transparent focus:border-blue-500/50 rounded-2xl font-bold text-base outline-none shadow-inner text-slate-900 dark:text-white transition-all" 
      />
    </div>
    {unitOptions && (
      <select 
  ...
  return (
    <div className="mt-10 rounded-[2.5rem] bg-white dark:bg-slate-900 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-500 border border-slate-100 dark:border-slate-800 w-full">
      <div className={`p-10 ${result.isCritical ? 'bg-red-600' : 'bg-blue-600'} text-white`}>
        <span className="inline-block bg-white/20 text-[10px] font-black px-6 py-2 rounded-full border border-white/30 uppercase tracking-widest mb-6">{result.status}</span>
      ...
      <div className="p-8 space-y-8">
        ...
        <div className="bg-slate-50 dark:bg-slate-800/40 p-8 rounded-[2rem] border border-slate-100 dark:border-slate-800">
           <div className="flex items-center gap-2 mb-3 text-blue-500"><BookOpen size={20} /><span className="text-[10px] font-black uppercase tracking-wider">Simulation Pearl</span></div>
           <p className="text-[14px] font-bold text-slate-500 dark:text-slate-400 italic leading-loose">"{result.masteryInsight}"</p>
        </div>
      </div>
    </div>
  );
